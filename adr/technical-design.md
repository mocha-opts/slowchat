...
P7 不引入第二套消息写入链路。回复、转发、撤回和 Reaction 均由 Messages Command Service 在 PostgreSQL 事务中完成，Controller/Gateway 只负责 Schema 解析、身份提取和 ACK 映射。

- 回复引用要求目标消息存在且位于同一 `conversation_id`；转发要求源消息对当前用户可见，目标发送再次执行成员、Block、禁言、附件和 Client Message ID 校验。本轮只转发 TEXT，源消息 ID 保存在新消息的 `forward_from_message_id`，不会复制源 Payload 作为事实。
- 撤回先锁定消息行再复核 Session、成员和角色：发送者只能在 `MESSAGE_RECALL_WINDOW_SECONDS` 内撤回自己的消息，GROUP `OWNER/ADMIN` 可撤回成员消息。状态更新与 `message.recalled.v1` Outbox 记录同事务提交；重复调用由 `recalled_at` 状态映射为稳定冲突码。
- Reaction 使用 `message_reactions` 的唯一键 `(message_id,user_id,reaction)`。插入采用 `ON CONFLICT DO NOTHING`，因此并发或重试只返回同一事实，不重复写 `message.reaction.updated.v1`；删除只允许拥有该 Reaction 的用户执行。
- `message_user_hides` 是用户视图投影边界，而不是消息删除。历史、单条查询和搜索按当前用户 LEFT JOIN 过滤隐藏记录；`clearBeforeSeq` 继续保存在 `conversation_user_states`，清空历史只单调推进该游标并清零个人未读计数。
- 基础搜索使用 PostgreSQL `ILIKE` 与 `(created_at,id)` 排序 Cursor；GIN tsvector 索引为后续全文搜索保留扩展点，P7 不引入 Elasticsearch。Cursor 内绑定查询串和版本，篡改或跨查询复用返回 `SEARCH_QUERY_INVALID`。

P7 领域事件继续复用 Outbox、RabbitMQ、Consumer Inbox、Realtime Dispatch 和 Sync Projection。事件 Payload 只包含消息/Reaction/用户视图所需字段，不包含对象存储 Key、凭证或其他敏感信息；Realtime 通知重复传输时依靠稳定 `eventId` 去重，数据库副作用只发生一次。
