...
DELETE /conversations/:conversationId/history
GET    /messages/search?q=<text>&cursor=<opaque>&limit=<n>
```

发送消息仍使用 `contentVersion=1`。TEXT/IMAGE/FILE 请求可选携带 `replyToMessageId` 和 `forwardFromMessageId`；回复目标必须属于当前会话，转发源必须是当前用户可访问的会话消息。转发在目标会话重新走标准 Seq、权限、Client Message ID 和 Outbox 事务，本轮只支持转发 TEXT。

- 撤回只改变消息的 `recalledAt/recalledBy` 状态并发布 `message.recalled.v1`；发送者受 `MESSAGE_RECALL_WINDOW_SECONDS`（默认 120 秒）限制，GROUP 中 `OWNER/ADMIN` 可撤回成员消息。重复撤回返回 `MESSAGE_ALREADY_RECALLED`，编辑仍是 POST-MVP。
- Reaction 以 `(message_id,user_id,reaction)` 唯一约束幂等；重复添加返回原 Reaction，不重复产生领域事件。新增/删除分别发布 `message.reaction.updated.v1`，合法成员可读取并操作当前会话消息。
- `DELETE /messages/:messageId/view` 写入当前用户的 `message_user_hides`，不会删除消息、影响其他成员或改变会话 Seq；重复隐藏保持幂等。`DELETE /conversations/:conversationId/history` 将当前用户的 `clearBeforeSeq` 推进到当前 `lastSeq`，只清空个人历史视图。
- 基础搜索只检索当前用户可见的 TEXT，使用 `(createdAt,messageId)` 不透明 Cursor，禁止 Offset；隐藏消息和 `clearBeforeSeq` 之前的消息不返回。查询串、Cursor 非法时返回 `SEARCH_QUERY_INVALID`。
- P7 Realtime 命令增加 `message.recall`、`reaction.add`、`reaction.remove`，每条命令返回 `WsAck`；领域事件经 Realtime Dispatch 映射为 `message.recalled`、`message.reaction.updated`、`message.hidden`，丢失通知由 Sync API 恢复。
